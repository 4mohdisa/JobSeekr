...
BULLETS_SLOT = "bullets"
"""The one ai slot that is not a paragraph of prose.

``ai.bullets`` is a list of lists — one list of rewritten bullet points per
experience row — so it is generated by its own path in ``documents/build.py``
rather than by ``generate_ai_slots``. It is declared here anyway because this
dict is what ``find_ai_slots`` and the template editor's autocomplete read: a
slot missing from it is reported as unknown and silently dropped.
"""
...
SLOT_SPECS: dict[str, AISlot] = {
    ...
    BULLETS_SLOT: AISlot(
        name=BULLETS_SLOT,
        max_words=28,
        tone="factual, active voice, no filler",
        instruction=(
            "Rewrite ONE of the candidate's own resume bullet points so that it speaks "
            "to this advertisement. Keep every fact the bullet already states, add "
            "none, and drop none. If the bullet already answers the ad, return it "
            "unchanged."
        ),
    ),
}
...
# Any read of the ai namespace, from a substitution or a block expression.
# Two regexes rather than one: the tag payload is isolated first so the word
# "ai." in a LaTeX comment is not mistaken for a slot, and every reference
# inside a payload is then found — a single tag can read two of them.
_TAG_PAYLOAD_RE = re.compile(r"\\(?:VAR|BLOCK)\{([^}]*)\}")
_AI_FIELD_RE = re.compile(r"\bai\.([A-Za-z_]\w*)")
...
# Names a template binds for itself. Without these, every `\VAR{role.title}`
# inside a `\BLOCK{for role in profile.experience}` was reported to the user as
# "'role' is not a known namespace" — the shipped resume template alone raised
# fourteen of them, so the editor's error list was pure noise and a real typo
# had nowhere to stand out.
_BOUND_NAME_RE = re.compile(
    r"\\BLOCK\{\s*(?:for\s+([A-Za-z_]\w*(?:\s*,\s*[A-Za-z_]\w*)*)\s+in\b"
    r"|set\s+([A-Za-z_]\w*)\s*=)"
)


def _bound_names(body: str) -> set[str]:
    """For-loop targets and set assignments — locals, not unknown namespaces."""
    names: set[str] = set()
    for match in _BOUND_NAME_RE.finditer(body):
        captured = match.group(1) or match.group(2) or ""
        names.update(part.strip() for part in captured.split(",") if part.strip())
    return names
...
def preview_ai_context(experience: Any = ()) -> dict[str, Any]:
    """Placeholder values for every ai slot, for a render that pays no LLM call.

    Lives here rather than in the preview endpoint because ``ai.bullets`` is not
    shaped like the prose slots — it is one list of bullets per experience row —
    and anything building this dict by hand gets that wrong. The template
    editor's preview and the document tests both render the shipped templates;
    if they disagree about the shape, one of them is not exercising what ships.
    """
    values: dict[str, Any] = {
        name: f"[{name} — generated per job]"
        for name in SLOT_SPECS
        if name != BULLETS_SLOT
    }
    # Each row's OWN highlights, not a placeholder. That is what production
    # renders whenever generation is unavailable or produces something the
    # fabrication check rejects, so it is the honest preview — the model
    # rewrites these bullets toward an ad, it does not replace them with
    # unrelated text — and it keeps the document tests exercising the real
    # strings from the profile rather than a stand-in.
    values[BULLETS_SLOT] = [
        [str(highlight) for highlight in (row.get("highlights") or [])]
...
def find_ai_slots(body: str) -> list[AISlot]:
    ...
    Block expressions count, not just substitutions. ``ai.bullets`` is a list of
    lists, so the resume reads it as ``\\BLOCK{for bullet in ai.bullets[i]}`` and
    never through a ``\\VAR``; scanning substitutions alone reported the resume
    as using no slots at all, and the build then rendered it with no ``bullets``
    key at all — which under StrictUndefined is a failed build, not a quiet one.
    """
    seen: list[str] = []
    for payload in _TAG_PAYLOAD_RE.finditer(body):
        for field in _AI_FIELD_RE.findall(payload.group(1)):
